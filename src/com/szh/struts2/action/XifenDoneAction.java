...
public class XifenDoneAction extends ActionSupport{
	...
	public String addXf()throws Exception{
		...
		System.out.println("在细分中名称"+fenlei[i]);
		List<Data> xlist=Database.queryXf2(name[i],fenlei[i],Integer.parseInt(id));
		for(int j=0;j<xlist.size();j++){
		System.out.println("xfdoneaction中："+xlist.get(j).getXf());
		System.out.println("xfdoneaction中："+xlist.get(j).getGsxf());
		}
